...
import jxlpoi.Cell;
import jxlpoi.CellFeatures;
import jxlpoi.CellReferenceHelper;
...
import jxlpoi.biff.CellFinder;
import jxlpoi.biff.ConditionalFormat;
import jxlpoi.biff.DVParser;
import jxlpoi.biff.DataValidation;
import jxlpoi.biff.EmptyCell;
...
import jxlpoi.biff.WorkspaceInformationRecord;
...
import jxlpoi.format.PageOrientation;
import jxlpoi.format.PaperSize;
...
import jxlpoi.write.WritableCellFeatures;
...
class WritableSheetImpl implements WritableSheet
{
                           ...
                           WritableWorkbookImpl ww)
  {
    ...
    poiSheet = ww.getPOIWorkbook().createSheet(n);
  }
    ...
    throws WriteException, RowsExceededException
  {
    ...
    if (poiSheet.getRow(row) == null) {
    	poiSheet.createRow(row);
    }
    XSSFRow poiRow = poiSheet.getRow(row);
    if (poiRow.getCell(column) == null) {
    	poiRow.createCell(column, translateCellType(cell));
    }
    org.apache.poi.xssf.usermodel.XSSFCell poiCell = poiRow.getCell(column);
    poiCell.setCellValue(cv.getContents());

    CellFormat cf = cell.getCellFormat();
    
    XSSFCellStyle cs = workbook.addCellStyle(cf);
    poiCell.setCellStyle(cs);
   ...
   * @exception jxlpoi.write.biff.RowsExceededException
   * @deprecated use the override which takes a CellView object
   */
