...
import jxlpoi.BooleanCell;
import jxlpoi.Cell;
import jxlpoi.CellType;
import jxlpoi.CellView;
import jxlpoi.DateCell;
import jxlpoi.HeaderFooter;
import jxlpoi.Hyperlink;
import jxlpoi.Image;
import jxlpoi.LabelCell;
import jxlpoi.NumberCell;
import jxlpoi.Range;
import jxlpoi.Sheet;
import jxlpoi.SheetSettings;
import jxlpoi.WorkbookSettings;
import jxlpoi.biff.AutoFilter;
import jxlpoi.biff.CellReferenceHelper;
import jxlpoi.biff.ConditionalFormat;
import jxlpoi.biff.DataValidation;
import jxlpoi.biff.FormattingRecords;
import jxlpoi.biff.FormulaData;
import jxlpoi.biff.IndexMapping;
import jxlpoi.biff.NumFormatRecordsException;
import jxlpoi.biff.SheetRangeImpl;
import jxlpoi.biff.XFRecord;
import jxlpoi.biff.drawing.Chart;
import jxlpoi.biff.drawing.CheckBox;
import jxlpoi.biff.drawing.ComboBox;
import jxlpoi.biff.drawing.Drawing;
import jxlpoi.biff.drawing.DrawingGroupObject;
import jxlpoi.biff.formula.FormulaException;
import jxlpoi.common.Assert;
import jxlpoi.common.Logger;
import jxlpoi.format.CellFormat;
import jxlpoi.read.biff.NameRecord;
import jxlpoi.read.biff.SheetImpl;
import jxlpoi.read.biff.WorkbookParser;
import jxlpoi.write.Blank;
import jxlpoi.write.Boolean;
import jxlpoi.write.DateTime;
import jxlpoi.write.Formula;
import jxlpoi.write.Label;
import jxlpoi.write.Number;
import jxlpoi.write.WritableCell;
import jxlpoi.write.WritableCellFormat;
import jxlpoi.write.WritableFont;
import jxlpoi.write.WritableHyperlink;
import jxlpoi.write.WritableImage;
import jxlpoi.write.WritableSheet;
import jxlpoi.write.WritableWorkbook;
import jxlpoi.write.WriteException;
...
class SheetCopier
{
  ...
  public void copySheet()
  {
    ...
    jxlpoi.read.biff.ColumnInfoRecord[] readCirs = fromSheet.getColumnInfos();

    for (int i = 0 ; i < readCirs.length; i++)
    {
      jxlpoi.read.biff.ColumnInfoRecord rcir = readCirs[i];
    ...
    try
    {
      jxlpoi.read.biff.RowRecord[] rowprops  = fromSheet.getRowProperties();
    ...
    {
      if (dr[i] instanceof jxlpoi.biff.drawing.Drawing)
      {
      ...
      else if (dr[i] instanceof jxlpoi.biff.drawing.Comment)
      {
        jxlpoi.biff.drawing.Comment c = 
          new jxlpoi.biff.drawing.Comment(dr[i], 
      ...
      else if (dr[i] instanceof jxlpoi.biff.drawing.Button)
      {
        jxlpoi.biff.drawing.Button b = 
          new jxlpoi.biff.drawing.Button
          (dr[i], 
           toSheet.getWorkbook().getDrawingGroup(),
           workbookSettings);
        drawings.add(b);
      }
      else if (dr[i] instanceof jxlpoi.biff.drawing.ComboBox)
      {
        jxlpoi.biff.drawing.ComboBox cb = 
          new jxlpoi.biff.drawing.ComboBox
          (dr[i], 
           toSheet.getWorkbook().getDrawingGroup(), 
           workbookSettings);
        drawings.add(cb);
      }
      else if (dr[i] instanceof jxlpoi.biff.drawing.CheckBox)
      {
        jxlpoi.biff.drawing.CheckBox cb = 
          new jxlpoi.biff.drawing.CheckBox
          (dr[i], 
           toSheet.getWorkbook().getDrawingGroup(), 
           workbookSettings);
        drawings.add(cb);
  ...
  public void importSheet()
  {
    ...
    try
    {
      ...
      for (int i = 0; i < rowprops.length; i++)
      {
        ...
        jxlpoi.read.biff.RowRecord rowrec = rowprops[i];
